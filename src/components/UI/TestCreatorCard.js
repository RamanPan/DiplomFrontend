...
const TestCreatorCard = (props) => {
    ...
    const handleDelete = () => {
      deleteReq(API_DELETE_TEST,{"id": props.id}).then(r => {
          navigate("/lk")
      })
    }
    const handleAgree = () => {
    ...
    return (
        <div>
                <Card sx={{mt:4,ml:6.251, width: 360, minHeight: 450, borderRadius: "15px",border:"10px", borderColor:"#9F4636", backgroundColor: "#F1DCC9" }}>
                    <CardMedia
                        component="img"
                        height="250"
                    ...
                    <Grid container sx = {{ml: 2, display: 'flex',alignItems: 'flex-start',}}>
                        <Button onClick={handleUpdate} fullWidth variant="contained" color="primary" sx={{width: 330, mt: 1,borderRadius:"10px"}}>Редактировать</Button>
                        {deleteState ? (<Button onClick={handleDelete} fullWidth variant="contained" sx={{backgroundColor : "#490005",width: 330, mt: 1,mb:1,borderRadius:"10px"}}>Вы уверены?</Button>) :
                        (<Button onClick={handleAgree} fullWidth variant="contained" sx={{backgroundColor : "#490005",width: 330, mt: 1,mb:1,borderRadius:"10px"}}>Удалить</Button>)}
                    </Grid>
