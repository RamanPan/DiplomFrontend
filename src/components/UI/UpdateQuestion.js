...
import {ID_QUESTIONS, QUE_ID} from "./Question";
...
const UpdateQuestion = (props) => {
    ...
    if(isFirst) {
        answerStore.birth({"id" : props.quest.id}).then(r => {
    ...
    const handleClickAddOrDeleteQuestion = () => {
        if(!switchBut) {
            ...
            deleteReq(API_DELETE_QUESTION,{"id": id}).then(response => {
                setSwitch(false)
            })
        }
    }
    const handleShowAnswer = () => {
            if (type === questionTypes[1]) {
                setAnswers([
                    <Answer number={1} sx={{mb: 2}} id_quest={ID_UPDATE_QUESTIONS[props.quest.number-1]} id_num={props.number}/>
                ])
            } else {
                setAnswers([
    ...
    return (
        ...
        > <Grid container sx={{ml: 2,mt: 2}}>
            ...
            <Grid sx = {{maxWidth:800,minHeight:300,}}>
                ...
                <TextField variant='outlined' name = "statement" defaultValue={props.quest.statement}  onChange={updateStatement} label = 'Введите текст вопроса(до 200 символов)' sx = {{backgroundColor: '#FFFFFF',mt: 1,mr:10, width:700, borderRadius: "8px",}}/>
